...
def test_pallas_pcr_matches_vmapped_thomas_in_interpret_mode():
    batch_size = 128
    n = 8
    batch = jnp.arange(batch_size, dtype=jnp.float32)[:, None]
    x = jnp.arange(n, dtype=jnp.float32)[None, :]
    ...
    a00 = 4.0 + 0.05 * x + 0.0003 * batch
    a01 = -0.8 - 0.01 * x + 0.0002 * batch
    a10 = -0.7 + 0.015 * x - 0.00025 * batch
    a11 = 5.0 + 0.04 * x + 0.0004 * batch
    off0 = -0.08 - 0.006 * edge - 0.0001 * batch
    off1 = -0.06 - 0.004 * edge - 0.00012 * batch
    rhs0 = jnp.sin(0.2 * x + 0.01 * batch)
    rhs1 = jnp.cos(0.15 * x - 0.008 * batch)
    ...
    pallas0, pallas1 = solve_block_tridiagonal_2x2_pallas_pcr_batched(
        a00,
        a01,
        a10,
        a11,
