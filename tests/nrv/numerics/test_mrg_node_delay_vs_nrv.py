...
from axonfleet import AxonInstance, ms, um
...
def test_mrg_node_delay_vs_nrv(diameter_um: float, threshold_mV: float, rmse_tol_ms: float) -> None:
    ...
    res = run_axonfleet_simulation(sim, tsim=4.0, dt=0.005)
    ...
    tc_as = crossing_times(vm_as, t_as, threshold_mV)
    tc_nrv = crossing_times(vm_nrv_i, t_as, threshold_mV)
    ...
    axs[0].plot(x_as, tc_as, "o-", lw=1.8, label="AxonFleet")
    ...
    axs[1].plot(d_as, y_as, "o-", lw=1.8, label="AxonFleet")
