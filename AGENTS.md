...
instructions: |
  ...
  - Intrinsic axon coordinates belong in AxonScope. World coordinates,
    ...
    core architecture; examples must convert them to sampled extracellular
    footprints, drives, and stimulations before creating solver-facing
    simulations.
  ...
  - The project is pre-release with one active user. Prefer clean deletion and
    direct design convergence over downstream compatibility, deprecation shims,
    aliases, or dual public paths.
  - For membrane authoring, user-facing vocabulary is "membrane model",
    "equation", "parameter", "gate", "current", and "observable". "Model IR"
    and "intermediate representation" are internal compiler/runtime terms, not
    concepts users should need to access directly.
  - Built-in membrane model truth lives in `src/axonscope/membranes/models/`:
    each model `.py` file must be the unique source of information for that
    model, including equations, unit-bearing defaults, public aliases, and
    derived parameter logic. `membranes/builtins.py`, `model_ir/`, solvers, and
    backends must not reintroduce model-family defaults or formulas.
  - Built-in axon templates may keep their ergonomic model-specific keyword API,
    but those kwargs must be forwarded only to the membrane descriptor/source
    compiler. Do not put model defaults, aliases, equations, derived parameter
    formulas, or unit conversion contracts in axon templates.
  ...
  3. **Execution Layer**: Solvers, backend execution facades, Model IR lowering, Dispatcher
  4. **Result Layer**: Analysis, Visualization, Data Management
  
  ## Key Technologies
  - **JAX** >=0.10.1,<0.11.0: JIT compilation, GPU acceleration, batch execution
  ...
  ### Design Patterns
  - **Frozen Dataclasses**: Use for immutable value objects (SolverOptions, KernelResult, private execution records)
  ...
  | `membranes/` | Public built-in and user-authored membrane model descriptions |
  | `model_ir/` | Internal runtime-agnostic membrane semantics, validation, hashes, fusion contracts |
  | `backends/jax/` | JAX lowering, runtime preparation, batch kernels, backend-local membrane execution |
  | `stimulation/` | Waveforms, clamps, extracellular footprints, drives, and stimulation objects |
  | `solvers/` | Numerical solving (Crank-Nicholson main path) |
  | `dispatcher/` | Pool planning, automatic batching logic |
  | `results/` | Public simulation results, one-axon views, recording manifests, compact observations, visualization |
  ...
  - Include public result diagnostics or benchmark metadata for reproducibility
  ...
  - **Example Coverage Rule**: Every public option, possibility, feature,
    advanced concept, runtime mode, inspection view, analysis workflow, or
    solver-facing user concept must be documented or described in a runnable
    didactic example. If no example documents it, either add/update the example
    in the same change or remove/archive the feature as useless, unclear, or
    legacy. Only skip example coverage when the concept is purely internal, and
    make that explicit in the final notes.
  ...
  AxonScope is pre-release and currently has one active user. Do not preserve
  retrocompatibility, downstream migration paths, compatibility aliases,
  deprecated wrappers, or old argument names. When an interface is confusing,
  replace it directly with the cleaner public API and update tests, docs,
  examples, and changelog together. Treat temporary compatibility as a bug
  ...
  been checked. Keep `AGENTS.md` focused on working rules; do not duplicate the
  ...
  - Vext optimization direction: keep the public stimulation API stable, but
    ...
    path is static-footprint single-cable execution with
    `current_mid_A[B,K,Nt] + footprint_mV_per_A[B,K,Nx]`, with squeezed rank-1
    forms allowed internally; do not re-densify compatible single-cable
    observer or recorded-Vm paths. Observer-only compact paths should reject
    unsupported imported/dynamic-dense stimulation shapes instead of silently
    falling back to dense `Vstim[B,Nt,Nx]`.
  ...
  - Solver pipeline inspection direction: planning, dispatch/batch, preparation,
    ...
    result assembly should each be explainable. `AxonSimulation.inspect()` now
    covers printable host-side planning, dispatch/batch, preparation, lowering,
    kernel, and result-assembly
  ...
  - [ ] No temporary compatibility aliases, migration shims, or deprecated wrappers unless explicitly requested by the user
  ...
  class KernelResult:
      """Immutable backend payload returned before public result assembly."""
      Vm: Any | None
      t: Any
...
rules:
  ...
  - name: Prefer clean pre-release APIs over compatibility shims
    description: AxonScope has one active user for now; replace confusing temporary APIs with clean user-facing interfaces and do not preserve backwards compatibility, aliases, or downstream migration paths unless explicitly requested
