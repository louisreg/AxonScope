...
from benchmark.runtime.pool_batch_demo import (
    build_pool_inputs,
    run_pool_mode,
)
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark pool batch runtime and dominant tensor sizes."
    )
    ...
    prefix = args.prefix or datetime.now().strftime("pool_memory_%Y%m%d_%H%M%S")
    json_path, csv_path = write_rows(
        rows,
        args.out_dir,
        prefix=prefix,
        metadata={
            "benchmark": "pool_memory",
    ...
    print("=== Pool memory/runtime benchmark ===")
    for row in rows:
