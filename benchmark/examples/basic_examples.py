"""Benchmark runnable basic examples as public workflow smoke/perf gates."""
...
DEFAULT_OUTPUT = REPO_ROOT / "benchmark" / "results" / "basic_examples"
...
EXAMPLES = {
    "06": ExampleSpec(
        key="06",
        path=REPO_ROOT / "examples" / "basic" / "06_activation_velocity.py",
        label="activation_velocity",
    ),
    "07": ExampleSpec(
        key="07",
        path=REPO_ROOT / "examples" / "basic" / "07_threshold_vs_diameter.py",
        label="threshold_vs_diameter",
    ),
    "08": ExampleSpec(
        key="08",
        path=REPO_ROOT / "examples" / "basic" / "08_recruitment_curve_population.py",
        label="recruitment_curve_population",
    ),
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--examples", default="06,07,08")
...
def main(argv: list[str] | None = None) -> int:
    ...
    _write_manifest(output, args, specs)

    if args.dry_run:
        _write_cases(output, args, specs)
        print(f"dry-run: basic_examples -> {output}")
        return 0
    ...
    for spec in specs:
        ...
        for phase, repeat in phase_plan:
            row = _run_one(module, spec, phase=phase, repeat=repeat, args=args, output=output)
            rows.append(row)
...
def _parse_examples(value: str) -> list[ExampleSpec]:
    ...
    for raw in value.split(","):
        ...
        if key not in EXAMPLES:
            ...
            raise SystemExit(f"Unknown basic example {key!r}; expected one of: {allowed}.")
...
def _run_one(module: Any, spec: ExampleSpec, *, phase: str, repeat: int, args: argparse.Namespace, output: Path) -> dict[str, Any]:
...
def _write_cases(output: Path, args: argparse.Namespace, specs: list[ExampleSpec]) -> None:
    with (output / "cases.csv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=("script", "preset", "platform", "example", "label"))
        writer.writeheader()
        for spec in specs:
            writer.writerow(
                {
                    "script": "basic_examples",
                    "preset": args.preset,
...
def _write_manifest(output: Path, args: argparse.Namespace, specs: list[ExampleSpec]) -> None:
    payload = {
        "script": "basic_examples",
        "preset": args.preset,
        "platform": args.platform,
