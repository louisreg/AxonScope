import torch
import matplotlib.pyplot as plt
import settings as s
import utils as u
from axonscope.benchmark import minibench
import solver_helper_torch as sh 
import numpy as np

# Device selection
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

@torch.compile
def run_torch(Nx):
    dx = s.L / (Nx - 1)
    dx_cm = dx * 1e-4
    dx2 = dx_cm**2
    x_axon = torch.linspace(0, s.L, Nx, device=device, dtype=torch.float64)
    alpha = s.D * (s.dt / 2.0) / dx2
    idx_inj = torch.argmin(torch.abs(x_axon - s.position))

    inj_uA_per_cm2 = s.amplitude * 1e-3 / (2.0 * torch.pi * s.a_cm * dx_cm)
    Nt = int(torch.ceil(torch.tensor(s.tsim / s.dt)).item())

    # Voltage arrays
    V = torch.full((Nx,), s.Vinit, device=device, dtype=torch.float64)
    V_all = torch.zeros((Nt, Nx), device=device, dtype=torch.float64)
    t_vec = torch.arange(0, Nt, device=device, dtype=torch.float64) * s.dt

    # Construct full tridiagonal matrix (dense)
    A = torch.zeros((Nx, Nx), device=device, dtype=torch.float64)
    for i in range(1, Nx-1):
        A[i, i-1] = -alpha
        A[i, i]   = 1.0 + 2.0*alpha
        A[i, i+1] = -alpha
    A[0,0] = 1.0
    A[-1,-1] = 1.0

    # Initialize gating variables
    minf, mtau, hinf, htau, ninf, ntau = sh.rates(torch.full((Nx,), s.Vinit, device=device, dtype=torch.float64))
    m_RA = minf.clone()
    h_RA = hinf.clone()
    n_RA = ninf.clone()

    for n in range(Nt):
        t_mid = t_vec[n]

        # Current injection
        Iinj = sh.Iinj_uAcm2(t_mid, s.t_start_inj, s.t_stop_inj, Nx, inj_uA_per_cm2, idx_inj)

        # Right-hand side
        Iion_curr = sh.Iion(V, Nx, m_RA, n_RA, h_RA)
        rhs = V + (s.dt / (2.0 * s.Cm)) * (Iinj - Iion_curr)

        # Update gates
        m_RA, h_RA, n_RA = sh.half_step_gates(s.dt, V, Nx, m_RA, h_RA, n_RA)

        # Boundary conditions
        rhs[0] = s.Vinit
        rhs[-1] = s.Vinit

        # Solve linear system
        V_half = torch.linalg.solve(A, rhs)

        # Hines extrapolation
        V_new = 2.0 * V_half - V
        V_new = torch.clamp(V_new, -500.0, 500.0)
        V_new[0] = s.Vinit
        V_new[-1] = s.Vinit

        V_all[n, :] = V_new
        V = V_new

    return t_vec.cpu().numpy(), V_all.cpu().numpy()


# --------------------------
# Benchmark
# --------------------------
t_v = []
for Nx in s.Nx_v:
    res, t = minibench(run_torch, Nx, n_iter=s.n_iter)
    t_v.append(t)

df = u.res_to_df(s.Nx_v, t_v, label="torch_linsolve_compile")
u.append_to_csv(df)

t_vec, V_all = res

x_axon = torch.linspace(0, s.L, Nx).cpu().numpy()
x_positions = [s.L/4, s.L/3, s.L/2, 2*s.L/3, 3*s.L/4]
indices = [np.argmin(np.abs(x_axon - xp)) for xp in x_positions]

fig, ax = plt.subplots(1, figsize=(5,5))
for idx, xp in zip(indices, x_positions):
    ax.plot(t_vec, V_all[:, idx], label=f'x={xp:.3f} cm')
ax.set_xlabel("Time (ms)")
ax.set_ylabel("Voltage (mV)")
ax.legend()
plt.show()
