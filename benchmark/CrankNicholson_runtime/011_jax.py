...
# --------------------------
# Run JAX simulation for given Nx
# --------------------------
def run_jax(Nx, Nt, alpha, inj_uA_per_cm2):
...
for Nx in s.Nx_v:
    ...
    # wrap the JAX function properly without lambda capturing arguments
    def run_fn():
        return run_jax(Nx, Nt, alpha, inj_uA_per_cm2)
    
    #run_fn_jit = jax.jit(run_fn)
    #res, t = minibench(run_fn_jit, n_iter=s.n_iter)

    start = time.time()
    res = run_jax(Nx, Nt, alpha, inj_uA_per_cm2)
    res[1].block_until_ready()
    end = time.time()
    print("Execution time (no Jit):", end - start)
    t_v.append(end-start)


# Save benchmark results
df = u.res_to_df(s.Nx_v, t_v, label="jax_linsolve")
u.append_to_csv(df)
