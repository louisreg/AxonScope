...
def thomas_solve(a, b, c, d):
    ...
    def scan_bwd(x_next, i):
        x_i = d_[i] - c_[i]*x_next
        return x_i, x_i

    _, x_all = lax.scan(scan_bwd, d_[-1], jnp.arange(n-2, -1, -1))
    x = x_all[::-1]
    x = jnp.concatenate([x, jnp.array([d_[-1]])])
    return x

# --------------------------
# JAX Crank-Nicholson simulation using Thomas + lax.scan
# --------------------------
def run_jax_thomas_scan(Nx, Nt, alpha, inj_uA_per_cm2):
    x_axon = jnp.linspace(0.0, s.L, Nx)
    idx_inj = jnp.argmin(jnp.abs(x_axon - s.position))

    # Tridiagonal vectors - compute once
    ...
    def step(carry, _):
        ...
        return (V_new, m_RA, h_RA, n_RA), V_new  # V_new goes into scan output

    (V, m_RA, h_RA, n_RA), V_all = lax.scan(step, (V, m_RA, h_RA, n_RA), jnp.arange(Nt))
...
for Nx in s.Nx_v:
    ...
    run_fn_jit = jax.jit(lambda Nx=Nx, Nt=Nt, alpha=alpha, inj_uA_per_cm2=inj_uA_per_cm2:
                         run_jax_thomas_scan(Nx, Nt, alpha, inj_uA_per_cm2))
    ...
    print(f"Nx={Nx}: Execution time (Thomas scan jitted) = {end-start:.6f} s")
