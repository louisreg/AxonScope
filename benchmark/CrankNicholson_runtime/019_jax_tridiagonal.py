...
for Nx in s.Nx_v:
    ...
    res = run_jax_tridiagonal_scan(Nx, Nt, alpha, inj_uA_per_cm2)
    res[1].block_until_ready()
    end_time = time.perf_counter()

    print(f"Nx={Nx}: Execution time (tridiagonal) = {end_time - start_time:.4f} s")
    t_v.append(end_time - start_time)
    res_list.append(res)

# Save benchmark results
df = u.res_to_df(s.Nx_v, t_v, label="jax_tridiagonal")
u.append_to_csv(df)
