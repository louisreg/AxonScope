...
def thomas_solve(a, b, c, d):
    """Solve Ax = d for tridiagonal A using Thomas algorithm in JAX."""
    n = b.shape[0]
...
for Nx in s.Nx_v:
    ...
    start = time.time()
    res = run_jax_thomas(Nx, Nt, alpha, inj_uA_per_cm2)
    res[1].block_until_ready()
    end = time.time()
    print(f"Nx={Nx}: Execution time (Thomas no Jit) = {end-start:.4f} s")
    t_v.append(end-start)
    res_list.append(res)

# Save benchmark results
df = u.res_to_df(s.Nx_v, t_v, label="jax_thomas")
u.append_to_csv(df)
