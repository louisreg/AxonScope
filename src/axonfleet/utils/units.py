"""Unit helpers for the public AxonFleet API.

AxonFleet stores canonical plain floats internally, but user-facing interfaces
...
class QuantityLike(Protocol):
    """Minimal unit-bearing scalar/array protocol accepted by AxonFleet."""
...
def require_quantity(value: Any, spec: UnitSpec, *, name: str) -> float:
    ...
    expected physical dimension. Use it for public interfaces where AxonFleet
...
def get_unit_registry() -> Any:
    """Return AxonFleet's Pint unit registry.

    The registry is created lazily so importing AxonFleet still works in an
    ...
    try:
        ...
        raise UnitSupportError(
            ...
            "dependencies, or pass plain numeric values in AxonFleet's "
...
class _UnitRegistryProxy:
    """Lazy proxy so `axonfleet.units.ureg` is import-safe without Pint."""
...
def Q_(value: Any, units: str | None = None) -> Any:
    """Construct a Pint quantity with AxonFleet's unit registry."""
