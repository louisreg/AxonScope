...
from axonfleet.analysis.core import (
    ...
    AnalysisResult,
    AnalysisStatus,
    _AnalysisNotApplicableError,
    _MissingAnalysisInputError,
)
from axonfleet.analysis.activation import ActivationEvent, _detect_activation
from axonfleet.analysis.posthoc import _conduction_velocity
from axonfleet.positions import ALL, DISTAL, PositionSelector
...
def _missing(
    ...
    *,
    ...
    return _MissingAnalysisInputError(
        message,
...
def _require_membrane_voltage(row: Any, signal: Signal[Any]) -> np.ndarray:
    if not isinstance(signal, Signal):
        raise TypeError("analysis signal must be an axonfleet.Signal descriptor.")
    if signal.id != MEMBRANE_VOLTAGE.id:
        raise _AnalysisNotApplicableError(
...
def _selected_columns(
    ...
    *,
    ...
    if positions_um.shape != (vm.shape[1],):
        raise _MissingAnalysisInputError(
    ...
    for row in _result_rows(result):
        try:
            ...
            if _is_missing_input_error(exc):
                ...
                input_requirement = _MissingAnalysisInputError(
                    str(exc),
...
class Activation:
    ...
    def detect(self, result: Any) -> ActivationEvent:
        """Return the compact activation event for one axon result."""

        _require_membrane_voltage(result, self.signal)
        return _activation_event(
            result,
...
class VmRaster:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.target, PositionSelector):
            raise TypeError("target must be an axonfleet PositionSelector.")
...
class ConductionVelocity:
    ...
    def detect(self, result: Any) -> float:
        """Return one direct conduction-velocity estimate in meters/second."""
    ...
    def _evaluate_one(self, row: Any) -> tuple[float, AnalysisStatus, str, None]:
        try:
            _require_membrane_voltage(row, self.signal)
        except _MissingAnalysisInputError:
