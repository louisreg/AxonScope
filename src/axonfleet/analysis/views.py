...
from axonfleet.views.plotting import decorate_axis, ensure_axis
...
def analysis_result_rows(result: Any) -> tuple[dict[str, Any], ...]:
    ...
    rows = []
    unit_text_value = unit_text(result.unit)
        ...
        rows.append(
            {
                ...
                "value": display_value(values[row_index]),
                "status": getattr(status, "value", str(status)),
                "message": result.messages[row_index],
                "unit": unit_text_value,
            }
        )
    return tuple(rows)
...
def format_analysis_result(result: Any) -> str:
    ...
    unit_text_value = unit_text(result.unit) or "-"
    title = (
        f"{result.name}: valid={pop.n_valid}/{pop.n_total}, "
        f"applicable={pop.n_applicable}, failed={pop.n_failed}, unit={unit_text_value}"
    )
    rows = []
...
def plot_analysis_result(
    ...
    **plot_kwargs: Any,
    ...
    if x_unit is not None and x_label == "row":
        x_label = f"row [{unit_text(x_unit)}]"
    return decorate_axis(
        ...
        ylabel=(
            ...
            or (
                result.name
                if not unit_text_value
                else f"{result.name} [{unit_text_value}]"
            )
        ),
        title=title,
        grid=grid,
...
def plot_spike_raster(
    ...
    **line_kwargs: Any,
    ...
    """Plot spikes detected by :func:`axonfleet.analysis.rasterize`."""
