...
from axonfleet.cache import writable_directory
    ...
    *,
    ...
    if not cache_hit:
        ...
        (directory / "source_snapshot.py").write_text(
            "# Generated from a validated AxonFleet compiled membrane graph.\n",
            encoding="utf-8",
        )
...
def _unsupported_call_statement_message(node: ast.Call) -> str | None:
    ...
    if _call_base_name(node.func) in _ARRAY_RUNTIME_MODULES:
        return (
            ...
            "Use supported helpers from axonfleet.membranes.math."
        )
        ...
        if not isinstance(node.func, ast.Name):
            if _call_base_name(node.func) in _ARRAY_RUNTIME_MODULES:
                raise SourceModelCompileError(
                    ...
                    "equations. Use supported helpers from axonfleet.membranes.math."
                )
            raise SourceModelCompileError(
                "Only direct helper calls from axonfleet.membranes.math are "
...
def _cache_root(cache_root: str | os.PathLike[str] | None) -> Path:
    ...
    return writable_directory("model_codegen")
...
def _load_generated_modules(
    ...
    *,
    ...
    for target in dict.fromkeys(targets):
        ...
        module_name = f"axonfleet_model_codegen_{key}_{target}"
...
def _generated_module_source(
    ...
    *,
    ...
    metadata = {
        **metadata,
        "runtime_currents": {current.name: {} for current in model.currents},
    }
    ...
    return (
        "# Generated by AxonFleet. Do not edit by hand.\n"
        ...
        + (
            ...
            f"RUNTIME_CONTRACT_VERSION = {_GENERATED_RUNTIME_CONTRACT_VERSION!r}\n"
        )
    )
...
def _generated_runtime_contract(
    ...
    *,
    ...
    step = model.step_program
    return {
        "version": _GENERATED_RUNTIME_CONTRACT_VERSION,
...
def _expression_arg_names(
    ...
    base = tuple(
        name
        for section in ("inputs", "states", "parameters", "runtime_currents")
