...
def _infer_intrinsic(
    ...
    if name in {"exp", "expm1", "log", "log1p", "safe_exp", "sigmoid", "tanh"}:
        ...
        return QuantitySpec(unit=DIMENSIONLESS, dtype=args[0].dtype)
    if name == "q10":
        if not is_dimensionless(args[0].unit):
        ...
        return QuantitySpec(unit=DIMENSIONLESS, dtype=args[1].dtype)
    if name in {"alpha_from_inf_tau", "beta_from_inf_tau"}:
        ...
        return QuantitySpec(unit=RATE_PER_MS, dtype=args[0].dtype)
    if name in {"abs", "sqrt"}:
        return args[0]
    if name == "clip":
