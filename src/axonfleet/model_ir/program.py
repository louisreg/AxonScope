...
from axonfleet.utils.units import CONDUCTANCE_DENSITY_MS_CM2
...
class MembraneProgram:
    ...
    This is not a public user-facing API. Users write membrane models; AxonFleet
...
def membrane_program_from_model_ir(model: ModelIR) -> MembraneProgram:
    ...
    current_groups = _group_name_indices(raw_current_names)
    conductance_groups = _group_name_indices(raw_conductance_names)
    parameter_units = {
        parameter.name: parameter.quantity.unit for parameter in model.parameters
    }
    conductance_parameter_names = tuple(
        _conductance_parameter_name(
            current.name,
            current.conductance,
            parameter_units,
        )
...
def _conductance_name(current_name: str) -> str:
    if current_name.startswith("I_"):
        return "g_" + current_name[2:]
    return f"g_{current_name}"


def _conductance_parameter_name(
    current_name: str,
    conductance: Expression,
    parameter_units: dict[str, str],
) -> str | None:
    for name in _symbol_names(conductance):
...
def _group_name_indices(names: tuple[str, ...]) -> _MembraneNameGroups:
    order: list[str] = []
    ...
    return _MembraneNameGroups(
        names=tuple(order),
        indices=tuple(tuple(groups[name]) for name in order),
    )


def _symbol_names(expr: Expression) -> tuple[str, ...]:
    if isinstance(expr, Literal):
        return ()
    if isinstance(expr, Symbol):
        return (expr.name,)
    if isinstance(expr, UnaryOp):
        return _symbol_names(expr.operand)
    if isinstance(expr, BinaryOp):
        return (*_symbol_names(expr.left), *_symbol_names(expr.right))
    if isinstance(expr, Call):
        names: list[str] = []
        for arg in expr.args:
            names.extend(_symbol_names(arg))
        return tuple(names)
    return ()
