...
class Device:
    ...
    @classmethod
    def auto(cls) -> "Device":
        """Let AxonFleet or the concrete runtime choose a device."""
...
@dataclass(frozen=True)
class SolverPolicy:
    """Typed double-cable solver-family request for executable simulations."""
...
class ExecutionPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.device, Device):
            raise TypeError("execution_policy.device must be an axonfleet.Device value.")
        if self.precision is not None and not isinstance(self.precision, PrecisionPolicy):
            raise TypeError(
                "execution_policy.precision must be an axonfleet.PrecisionPolicy value."
            )
        if self.solvers is not None and not isinstance(self.solvers, SolverPolicy):
            raise TypeError(
                "execution_policy.solvers must be an axonfleet.SolverPolicy value."
            )
...
def coerce_runtime(value: Any) -> Any:
    ...
    if not isinstance(value, RuntimeTarget):
        raise TypeError("runtime must be an axonfleet.runtime target value.")
