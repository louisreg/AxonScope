...
This module owns the decision from prepared AxonFleet rows to concrete kernel
...
from axonfleet.dispatcher.numeric_axis import (
...
import axonfleet.runtime.inputs.planning as input_planning
...
def supports_compact_double_cable_factorized(
    ...
    if previous is None:
        return False
    mode = _factorized_extracellular_mode(factorized)
    if not JAX_DOUBLE_CABLE_INPUT_CONTRACT.extracellular.supports(mode):
        return False
