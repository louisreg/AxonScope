...
from axonfleet.runtime.jax.recording.results import BatchKernelResult
...
def _resolve_double_cable_run_solver_settings(
    ...
    *,
    ...
    if solver_engine is None:
        raise ValueError("solver engine must be resolved before kernel dispatch.")
    normalized_platform = platform.lower()
    solver = solver_engine.double_cable_block_solver
    ...
    def run(
        ...
        *,
        ...
        with benchmark_span("kernel.prepare_inputs", mode="double", nx=nx, nt=grid.Nt):
            vext_mid = extracellular_potential_mid_mV
            if vext_mid is None:
                raise ValueError(
            ...
            if isinstance(vext_mid, FactorizedExtracellularPotentialBatch):
                ...
                if not supports_compact_double_cable_factorized(factorized_vext):
                    raise ValueError(
                        "double-cable factorized input must satisfy the compact "
                        "kernel contract; lower unsupported rows to dense first."
                    )
                vext_batch = None
            ...
            if factorized_vext is None:
                vext_previous = extracellular_potential_initial_previous_mV
            ...
            iinj_mid = intracellular_current_density_mid
            if iinj_mid is None:
                iinj_batch = None
            else:
            ...
            block_solver, tiled_thomas_block_b = (
                _resolve_double_cable_run_solver_settings(
                    self.solver_engine,
                    platform=_effective_double_cable_platform(self.solver_engine),
                )
            )
...
def _effective_double_cable_platform(solver_engine: JaxSolverEngine) -> str:
    if solver_engine.platform:
        return str(solver_engine.platform)
    raise ValueError("solver engine platform must be resolved before kernel dispatch.")
...
def _run_double_cable_batch_array_chunks(
    *,
    ...
    kernel_block_solver = double_cable_block_solver
