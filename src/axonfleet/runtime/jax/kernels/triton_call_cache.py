"""Persistent lowering cache for AxonFleet-owned jax-triton kernels."""
...
_cached_triton_kernel_call_p = jex.core.Primitive(
    "axonfleet_cached_triton_kernel_call"
)
...
def _cached_triton_kernel_call_lowering(
    ...
    *array_args: Any,
    ...
    if platform != "cuda":
        raise ValueError(
            f"AxonFleet's persistent Triton cache supports CUDA, got {platform!r}."
        )
...
def _raise_on_jvp(*args: Any, **kwargs: Any) -> None:
    del args, kwargs
    raise NotImplementedError("Cached AxonFleet Triton calls do not support JVP.")
...
def _batch_cached_triton_call(
    ...
    **params: Any,
    ...
    if not params.get("vmap_flatten_elements", False):
        raise NotImplementedError("Cached AxonFleet Triton call does not support vmap.")
    mapped = tuple(
...
def _triton_cache_root() -> Path:
    return writable_directory("runtime", "jax", "triton")
