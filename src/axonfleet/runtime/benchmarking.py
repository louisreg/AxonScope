...
class BenchmarkReport:
    ...
    def format(self) -> str:
        if not self.events:
            return "AxonFleet benchmark: no events recorded."
        lines = [
            f"AxonFleet benchmark: {len(self.events)} events",
...
class BenchmarkSession:
    ...
    def start_profile(self) -> None:
        ...
        if self.config.profile_runtime == "none":
            return
        try:
            from axonfleet.runtime.execution import benchmark_profile_start
    ...
    def stop_profile(self) -> None:
        if self._profile_handle is None:
            ...
            return
        try:
            from axonfleet.runtime.execution import benchmark_profile_stop
    ...
    def _device_memory_profile(self, name: str, event_id: int) -> dict[str, Any]:
        ...
        try:
            from axonfleet.runtime.execution import benchmark_save_device_memory_profile
...
def enable_benchmark(
    ...
    *,
    ...
    """Enable benchmark instrumentation for subsequent AxonFleet calls."""
    ...
    if active_benchmark_session() is not None:
        raise RuntimeError("An AxonFleet benchmark session is already active.")
...
def _collect_metadata(output_dir: Path, config: BenchmarkConfig) -> dict[str, Any]:
    ...
    try:
        from axonfleet.runtime.environment import collect_environment_info
...
def _current_rss_mib() -> float | None:
    try:
        from axonfleet.utils.progress_reporting import current_rss_mib
...
def _device_memory_snapshot() -> dict[str, Any]:
    from axonfleet.runtime.execution import benchmark_device_memory_snapshot
