"""User-facing views for AxonFleet simulation inspection records."""
...
from axonfleet.inspection.records import SimulationInspection
...
def format_simulation_inspection(report: SimulationInspection) -> str:
    ...
    lines = [
        "AxonFleet solver pipeline inspection",
        "planning:",
        (
    ...
    for group in report.dispatch_groups:
        ...
        lines.append(
            f"  group {group.group_id}: method={method}, {group.batch_kind}, "
    ...
    for prepared in report.preparations:
        lines.append(
            ...
            f"x_positions={prepared.x_positions_shape}, "
            f"representative={prepared.representative_index}"
        )
    ...
    for probes in report.probes:
        ...
        lines.append(
            ...
            f"max_probes={probes.max_probe_count}, "
...
def print_simulation_inspection(
    ...
    *,
    ...
    for column in ("group", "method", "kind", "rows", "Nx", "padding", "indices"):
        dispatch.add_column(column, overflow="fold")
    for group in report.dispatch_groups:
        dispatch.add_row(
            str(group.group_id),
            group.dispatch_method,
            group.batch_kind,
            str(group.size),
            str(group.nx),
    ...
    console.print(
        Panel(
            ...
            title="AxonFleet solver pipeline inspection",
            expand=False,
        )
    )
