...
from axonscope.summary_views import rows_to_dataframe
...
def format_simulation_estimate(estimate: Any) -> str:
    ...
    lines = [
        ...
        "groups:",
        "  id route  kind                    rows Nx  pad  rec->kernel  observer                 retained total",
    ]
    for group in estimate.groups:
        lines.append(
            "  "
            f"{group.group_id:<2d} {group.route:<6s} {group.batch_kind:<23s} "
    ...
    *,
    ...
    console.print(
        Panel(
            ...
            title="AxonScope simulation estimate",
            expand=False,
        )
    )
