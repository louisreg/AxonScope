...
def _unit_label(unit: Any, fallback: str = "microampere") -> str:
    return unit_label(unit, fallback=fallback)


def _unit_text(unit: Any, fallback: str = "microampere") -> str:
    return unit_text(unit, fallback=fallback)
...
def threshold_search_rows(
    result: Any,
    *,
    unit: Any = "microampere",
) -> tuple[dict[str, Any], ...]:
    """Return one row per tested threshold-search amplitude."""

    unit_label = _unit_label(unit)
    amplitudes = units.to_array(
        units.Q_([entry.amplitude_uA for entry in result.history], "microampere"),
        unit_label,
        dtype=float,
    )
    rows = []
    for iteration, (entry, amplitude) in enumerate(
        zip(result.history, amplitudes, strict=True),
    ):
        rows.append(
            {
                "iteration": int(iteration),
                "amplitude": float(amplitude),
                "activated": bool(entry.activated),
                "first_time_ms": entry.event.first_time_ms,
                "first_position_um": entry.event.first_position_um,
                "peak_mV": entry.event.peak_mV,
            }
        )
    return tuple(rows)


def threshold_search_to_dataframe(result: Any, *, unit: Any = "microampere") -> Any:
    """Return a pandas DataFrame for one threshold-search result."""

    return rows_to_dataframe(threshold_search_rows(result, unit=unit))


def format_threshold_search(result: Any, *, unit: Any = "microampere") -> str:
    """Return a compact threshold-search summary."""

    unit_label = _unit_label(unit)
    unit_text = _unit_text(unit)
    amplitude = (
        "none"
        if result.amplitude_uA is None
        else f"{float(units.to_array(result.amplitude, unit_label, dtype=float)):g} {unit_text}"
    )
    lines = [
        "AxonScope threshold search",
        (
            f"status={result.status}, amplitude={amplitude}, "
            f"iterations={result.n_iterations}"
        ),
    ]
    for row in threshold_search_rows(result, unit=unit):
        lines.append(
            f"  {row['iteration']:>2d}: amplitude={row['amplitude']:g} {unit_text}, "
            f"activated={row['activated']}"
        )
    return "\n".join(lines)
...
def print_threshold_search(
    ...
    *,
    ...
    """Print a compact threshold-search summary."""

    print_summary(format_threshold_search(result, unit=unit), file=file)
...
def plot_threshold_search(
    ...
    **plot_kwargs: Any,
    ...
    unit_text = _unit_text(unit)
    rows = threshold_search_rows(result, unit=unit)
    amplitudes = np.asarray([row["amplitude"] for row in rows], dtype=float)
    activated = np.asarray([row["activated"] for row in rows], dtype=float)
...
def recruitment_curve_rows(
    ...
    *,
    ...
    unit_label = _unit_label(unit)
    amplitudes = units.to_array(result.amplitudes, unit_label, dtype=float)
    return tuple(
        {
            "amplitude": float(amplitude),
            "count": int(count),
            ...
            rows.append(
                {
                    ...
                    "row": int(row_index),
                    "row_label": _display_value(row_label),
                    value_name: _display_value(value),
                    "observation": _display_value(result.observations[value_index, row_index]),
                }
            )
    return tuple(rows)
...
def threshold_curve_rows(
    ...
    *,
    ...
    y_unit = _unit_label(threshold_unit)
    ...
    return tuple(
        {
            row_name: _display_value(row_value),
...
def format_threshold_curve(
    ...
    *,
    ...
    threshold_text = _unit_text(threshold_unit)
    lines = [
        "AxonScope threshold curve",
...
def plot_threshold_curve(
    ...
    **plot_kwargs: Any,
    ...
    threshold_text = _unit_text(y_unit)
    plot_kwargs.setdefault("marker", "o")
