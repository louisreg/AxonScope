...
class SimulationEstimate:
    ...
    def rows(self, *, section: str = "items") -> tuple[dict[str, Any], ...]:
        ...
        from axonscope.performance_views import simulation_estimate_rows
    ...
    def to_dataframe(self, *, section: str = "items") -> Any:
        ...
        from axonscope.performance_views import simulation_estimate_to_dataframe
    ...
    def print(self, file: TextIO | None = None, *, rich: bool | None = None) -> None:
        ...
        from axonscope.performance_views import print_simulation_estimate
...
def _estimate_dispatch_group(
    ...
    *,
    ...
    if can_batch:
        ...
        has_intracellular = any(
            getattr(row, "intracellular_contexts", ()) for row in simulations
        )
        intracellular_format = (
            "dense" if has_intracellular else "zero_no_intracellular_context"
        )
        extracellular_format = (
            "dense" if has_extracellular else "zero_no_extracellular_stimulation"
        )
...
def _stimulation_rows(instances: Sequence[AxonInstance]) -> tuple[tuple[Any, ...], ...]:
    rows: list[tuple[Any, ...]] = []
    for instance in instances:
        rows.append(tuple(getattr(instance, "extracellular_stimulations", ())))
    return tuple(rows)


def _extracellular_stimulation_count(instances: Sequence[AxonInstance]) -> int:
    return sum(len(row) for row in _stimulation_rows(instances))


def _extracellular_drive_count(instances: Sequence[AxonInstance]) -> int:
    total = 0
    for row in _stimulation_rows(instances):
...
def _unique_stimulus_count(instances: Sequence[AxonInstance]) -> int:
    ...
    for instance in instances:
        ...
        for stimulation in getattr(instance, "extracellular_stimulations", ()):
