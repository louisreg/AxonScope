"""Schild 1997 DRG C-fiber membrane equations as standalone source."""
...
from axonscope.utils.units import degC, mM, mM_per_uA_cm2_ms, mS_per_cm2, mV, ms, per_ms_per_mM, uA_per_cm2, um
...
class Schild97(Model):
    """Schild 1997 DRG C-fiber membrane equations."""
    ...
    metadata = {
        "display_name": "Schild 1997 DRG C-fiber",
        "family": "schild",
        "source_reference": "Schild et al. 1997 DRG C-fiber membrane model with calcium dynamics",
    ...
    def naf(self, Vm: Voltage, celsius: Temperature = 37.0 * degC):
        """Fast sodium activation and inactivation."""

        qt_m_naf: Dimensionless = q10(2.30, celsius, 22.0 * degC)
        qt_h_naf: Dimensionless = q10(1.50, celsius, 22.0 * degC)
        tau_m_naf: Time = (
            1.15 * ms * exp(-((0.060 * ((Vm + 40.0 * mV) / (1.0 * mV))) ** 2))
            + 0.21 * ms
        ) / qt_m_naf
        inf_m_naf: Dimensionless = 1.0 / (1.0 + exp((-31.62 * mV - Vm) / (6.98 * mV)))
        ...
        tau_h_naf: Time = (
            18.0 * ms * exp(-((0.043 * ((Vm + 62.5 * mV) / (1.0 * mV))) ** 2))
            + 1.35 * ms
        ) / qt_h_naf
        inf_h_naf: Dimensionless = 1.0 / (1.0 + exp((-65.99 * mV - Vm) / (-5.97 * mV)))
        ...
        self.keep(alpha_m_naf, beta_m_naf, alpha_h_naf, beta_h_naf)
    ...
    def nas(self, Vm: Voltage, celsius: Temperature = 37.0 * degC):
        ...
        qt_m_nas: Dimensionless = q10(2.30, celsius, 22.0 * degC)
        qt_h_nas: Dimensionless = q10(1.50, celsius, 22.0 * degC)
        tau_m_nas: Time = (
            1.45 * ms * exp(-((0.058 * ((Vm + 14.5 * mV) / (1.0 * mV))) ** 2))
            + 0.26 * ms
        ) / qt_m_nas
        inf_m_nas: Dimensionless = 1.0 / (1.0 + exp((-11.29 * mV - Vm) / (5.54 * mV)))
        ...
        tau_h_nas: Time = (
            10.75 * ms * exp(-((0.067 * ((Vm + 13.5 * mV) / (1.0 * mV))) ** 2))
            + 3.15 * ms
        ) / qt_h_nas
        inf_h_nas: Dimensionless = 1.0 / (1.0 + exp((-31.0 * mV - Vm) / (-5.20 * mV)))
    ...
    def currents(self,
        ...
        cao: Concentration,
        c_kca: Dimensionless,
        gbna: ConductanceDensity = 0.018261 * mS_per_cm2,
        gbca: ConductanceDensity = 0.00913049 * mS_per_cm2,
        gbar_naf: ConductanceDensity = 22.434928 * mS_per_cm2,
        gbar_nas: ConductanceDensity = 22.434928 * mS_per_cm2,
        gbar_kd: ConductanceDensity = 1.956534 * mS_per_cm2,
        gbar_ka: ConductanceDensity = 1.304356 * mS_per_cm2,
        gbar_kds: ConductanceDensity = 0.782614 * mS_per_cm2,
        gbar_can: ConductanceDensity = 0.521743 * mS_per_cm2,
        gbar_cat: ConductanceDensity = 0.18261 * mS_per_cm2,
        gbar_kca: ConductanceDensity = 0.913049 * mS_per_cm2,
        ...
        g_naf: ConductanceDensity = gbar_naf * (m_naf ** 3) * h_naf
