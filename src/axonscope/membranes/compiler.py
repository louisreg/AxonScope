...
from axonscope.model_ir.composition import compose_model_ir
from axonscope.model_ir.schema import ModelIR
from axonscope.model_ir.source import SourceModelCompileResult, compile_model_source_file
...
def _derived_parameter_defaults(kind: str, params: dict[str, float]) -> dict[str, float]:
    return _derived_parameter_defaults_from_path(
        membrane_source_path(kind),
        params,
        model_class_name=None,
    )
...
@lru_cache(maxsize=None)
def _source_module(kind: str) -> Any:
    return _source_module_from_path(membrane_source_path(kind))
...
def _source_module_from_path(source_path: Path) -> Any:
    ...
    module_name = f"axonscope.membranes.models._source_{digest}"
