...
def local_point_source_context(
    ...
    *,
    ...
    `axon_y` and `axon_z` are external transverse offsets used only to shift the
    analytical electrode into the axon-local frame. The returned context can be
    attached to an `AxonInstance` with no world-coordinate placement.
    """
    ...
    local_electrode = PointSourceElectrode(
        x=units.Q_(electrode.x_um, "micrometer"),
