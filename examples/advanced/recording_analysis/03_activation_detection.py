"""Detect compact post-hoc activation events.

Run:
    python examples/advanced/recording_analysis/03_activation_detection.py

This example uses full Vm recording and calls `Activation.detect()` after the
solve. The same definition can also be passed to structured analysis and the
VmRaster observer path.
"""
...
def main() -> None:
    ...
    # position selectors.
    criteria = {
        "any compartment": axs.analysis.Activation(
            threshold=0.0 * axs.mV,
            blanking=0.5 * axs.ms,
            target=axs.positions.ALL,
        ),
        "proximal end": axs.analysis.Activation(
            threshold=0.0 * axs.mV,
            blanking=0.5 * axs.ms,
            target=axs.positions.PROXIMAL,
        ),
        "distal end": axs.analysis.Activation(
            threshold=0.0 * axs.mV,
            blanking=0.5 * axs.ms,
            target=axs.positions.DISTAL,
        ),
        "clamp center": axs.analysis.Activation(
            threshold=0.0 * axs.mV,
            blanking=0.5 * axs.ms,
